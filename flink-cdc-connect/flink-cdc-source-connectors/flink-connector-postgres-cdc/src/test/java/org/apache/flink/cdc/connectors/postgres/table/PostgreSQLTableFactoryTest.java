...
class PostgreSQLTableFactoryTest {
    ...
    void testCommonProperties() {
        ...
        PostgreSQLTableSource expectedSource =
                new PostgreSQLTableSource(
                        ...
                        PARTITION_DISCOVERY_POLL_INTERVAL.defaultValue());
    ...
    @Test
    void testPartitionRoutingRejectedWithoutIncrementalSnapshot() {
        Assertions.assertThatThrownBy(
                        () -> {
                            Map<String, String> properties = getAllOptions();
                            // scan.incremental.snapshot.enabled defaults to false in getAllOptions
    ...
    @Test
    void testPartitionRoutingOptions() {
        Map<String, String> properties = getAllOptions();
        properties.put("scan.incremental.snapshot.enabled", "true");
        properties.put("scan.include-partitioned-tables.enabled", "true");
        properties.put("scan.partition-discovery.poll-interval", "5min");
    ...
    @Test
    void qualifyTableListAddsSchemaToEachCommaSeparatedTable() {
        Assertions.assertThat(
                        PostgreSQLTableSource.qualifyTableList("public", "products, regular_users"))
                .containsExactly("public.products", "public.regular_users");
    }

    @Test
    void qualifyTableListPreservesAlreadyQualifiedCurrentSchemaPatterns() {
        Assertions.assertThat(
                        PostgreSQLTableSource.qualifyTableList(
                                "public", "public.products, products_.*"))
                .containsExactly("public.products", "public.products_.*");
    }

    @Test
    void qualifyTableListThrowsOnCrossSchemaQualifiedName() {
        Assertions.assertThatThrownBy(
                        () -> PostgreSQLTableSource.qualifyTableList("public", "analytics.events"))
    ...
    @Test
    void qualifyTableListThrowsOnCrossSchemaQualifiedNameWithinList() {
        Assertions.assertThatThrownBy(
                        () ->
                                PostgreSQLTableSource.qualifyTableList(
                                        "public", "products, analytics.events"))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("analytics.events");
    }

    @Test
    void qualifyTableListPrefixesUnqualifiedRegexPattern() {
        // Ensures that regex metacharacters in the table-name part do not get the entry
        // misclassified as a fully-qualified name (which would incorrectly throw).
        Assertions.assertThat(PostgreSQLTableSource.qualifyTableList("public", "products_.*"))
                .containsExactly("public.products_.*");
    }
