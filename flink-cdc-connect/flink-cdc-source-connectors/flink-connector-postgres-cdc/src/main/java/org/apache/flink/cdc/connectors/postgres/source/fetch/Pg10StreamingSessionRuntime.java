...
/**
 * Runtime container for Debezium session-scoped objects in PG10 streaming mode.
 *
 * <p>This class encapsulates all session-scoped resources that need to be built together and closed
 * as a unit. It holds references to the JDBC connection, replication connection, schema,
 * dispatcher, error handler, and other components required for streaming CDC.
 */
public final class Pg10StreamingSessionRuntime implements AutoCloseable {

    private static final Logger LOG = LoggerFactory.getLogger(Pg10StreamingSessionRuntime.class);
    ...
    private final Pg10CaptureState captureState;

    public Pg10StreamingSessionRuntime(
    ...
    public Pg10CaptureState getCaptureState() {
        return captureState;
    }
    ...
    private void closeReplicationConnectionQuietly() {
        try {
            ...
            LOG.warn("Failed to close replication connection", e);
        }
    }
}
