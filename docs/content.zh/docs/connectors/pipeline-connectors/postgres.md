...
<table class="colwidths-auto docutils">
    ...
    <tr>
      ...
      <td style="word-wrap: break-word;">(none)</td>
      <td>String</td>
      <td>服务器上安装的 Postgres 逻辑解码插件名称。支持的取值为 decoderbufs 和 pgoutput。</td>
    </tr> 
    <tr>
    ...
    <tr>
      ...
      <td>String</td>
      <td>Postgres CDC 消费者可选的启动模式，合法的模式为 "initial"、"latest-offset"、"committed-offset" 和 "snapshot"。</td>
    </tr>
    <tr>
...
2. 开启 `scan.include-partitioned-tables.enabled` 后，被捕获的分区父表会展开成物理子分区表用于快照切分和日志捕获，但输出记录和建表事件会使用分区根表标识。
...
<table class="colwidths-auto docutils">
   ...
   <tr>
    ...
    <tr>
      <td>INTERVAL [P]</td>
      <td>STRING（当 <code>debezium.interval.handling.mode</code> 设置为 string 时）</td>
    </tr>
    <tr>
      <td>BYTEA</td>
      <td>BYTES 或 STRING（当 <code>debezium.binary.handling.mode</code> 设置为 base64、base64-url-safe 或 hex 时）</td>
    </tr>
    <tr>
      <td>
        JSON<br>
...
<table class="colwidths-auto docutils">
    <thead>
      <tr>
        <th class="text-left">Postgres 中的空间数据</th>
        <th class="text-left">Flink 中转换后的 JSON 字符串</th>
      </tr>
    </thead>
    <tbody>
      <tr>
